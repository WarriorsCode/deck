...
func TestDepAlreadyRunning(t *testing.T) {
	deps := map[string]config.Dep{
		"myservice": {Check: "true", Start: config.StringOrList{"false"}},
	}
...
func TestDepFirstStrategyWorks(t *testing.T) {
	...
	deps := map[string]config.Dep{
		"myservice": {Check: "test -f " + marker, Start: config.StringOrList{"touch " + marker}},
	}
	err := EnsureDeps(context.Background(), ".", deps)
	require.NoError(t, err)
...
func TestDepAllStrategiesFail(t *testing.T) {
	deps := map[string]config.Dep{
		"myservice": {Check: "false", Start: config.StringOrList{"true"}},
	}
